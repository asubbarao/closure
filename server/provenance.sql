-- provenance.sql — chain-of-custody: crypto fingerprints + pdf_revisions + export lineage.
-- After ingest (needs documents). Builds on v_src_decisions. Extensions: crypto, pdf.
-- Shape: one CTAS snapshot + one unified view. No intermediate view stack.

INSTALL crypto FROM community; LOAD crypto;

-- Ingest-time custody (immutable for the boot lifetime).
...
SELECT c.document_id,
       ...
       c.filename,
       c.source_path,
       c.source_sha256 AS source_fingerprint,
       c.source_blake3,
       c.hash_algo,
       c.crypto_core_match,
