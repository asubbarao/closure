-- pdf_io.sql — OCR enrich for empty (scanned) pages, scan-status, redact geometry note.
-- After ingest; prefer before detect so OCR words feed suggestion CTAS.
...
WITH fn AS (
    ...
    FROM duckdb_functions() WHERE function_name = 'read_pdf_words' LIMIT 1
),
probe AS (
...
SELECT
    ...
    (coalesce((SELECT has_auto_ocr FROM fn), false)
     AND coalesce((SELECT ocr_words FROM probe), 0) > 0) AS ocr_available,
    CASE
        ...
        WHEN coalesce((SELECT ocr_words FROM probe), 0) = 0
            THEN 'auto_ocr returned 0 OCR words — install tesseract eng model'
        ELSE 'ocr ready'
    END AS ocr_status_note,
    now() AS probed_at;

CREATE OR REPLACE TABLE _empty_pages AS
SELECT p.document_id, p.page_no FROM pages p
WHERE NOT EXISTS (
    SELECT 1 FROM words w WHERE w.document_id = p.document_id AND w.page_no = p.page_no
);
...
SELECT a.document_id, a.case_id, a.filename, a.source_path, a.page_count,
       a.text_layer_pages, a.ocr_pages, a.scanned_gap_pages,
       a.native_word_count, a.ocr_word_count, a.total_word_count,
       0::BIGINT AS image_count, cap.ocr_available, cap.ocr_status_note,
       ...
       CASE WHEN a.native_word_count = 0 AND a.ocr_word_count > 0 THEN 'scanned · OCR'
            WHEN a.native_word_count = 0 AND a.ocr_word_count = 0 THEN 'scanned — no text layer'
            WHEN a.ocr_word_count > 0 THEN 'mixed · OCR' END AS scan_badge,
       CASE WHEN a.native_word_count = 0 AND a.ocr_word_count > 0 THEN 'b-blue'
            WHEN a.native_word_count = 0 AND a.ocr_word_count = 0 THEN 'b-rej'
            WHEN a.ocr_word_count > 0 THEN 'b-pend' ELSE 'b-gray' END AS scan_badge_class,
       CASE WHEN a.native_word_count = 0 AND a.ocr_word_count > 0
                THEN 'Image-only pages OCR''d (source=ocr). Review boxes before export.'
            WHEN a.native_word_count = 0 AND a.ocr_word_count = 0 AND NOT cap.ocr_available
                THEN 'Scanned PDF; OCR unavailable. ' || cap.ocr_status_note
            WHEN a.native_word_count = 0 AND a.ocr_word_count = 0
                THEN 'No text layer and OCR returned no words.' END AS scan_detail
FROM agg a, pdf_ocr_capability cap;

CREATE OR REPLACE ROUTE api_doc_scan GET '/api/documents/:id/scan' AS
SELECT document_id, case_id, filename, source_path, page_count,
...
FROM document_scan_status WHERE case_id = $id ORDER BY filename;
